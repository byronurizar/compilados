...
const { Usuario, bd, GrupoGastoUsuario } = require('../../store/db');
    ...
    new Strategy({
        ...
        async function (tokenPayload, cb) {
            ...
            where c.usuarioId=${usuarioId} and a.rolId in(
                ...
                let grupoGasto = await GrupoGastoUsuario.findOne({
                    where: { usuarioId, estadoId: 1 }
                });
                if (!grupoGasto) {
                    return cb(new Error('Usuario no autorizado'), false);
                }
                let { grupo_gastoId } = grupoGasto;
                const userInfo = user.dataValues;
                userInfo.actions = permisos;
                userInfo.grupo_gastoId = grupo_gastoId;
