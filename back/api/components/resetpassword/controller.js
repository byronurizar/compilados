    ...
    if (personaId > 0) {
        ...
        if (usuarioId > 0) {
            ...
            if (!solicitudExistente) {
                ...
                const { minutosVigenciaEnlace = 10 } = !!infoCorreo && infoCorreo;
                ...
                const resultEmail = await Correo.sendMailoAuth2(configuracion, email, "Restablecer Contraseña", null, htmlResetPassword);
                if (resultEmail.code === 0) {
                    let messageId = resultEmail.data.messageId;
                    ...
                    response.code = 0;
                    response.data = "No se logró enviar el correo para restablecer su contraseña, por favor intente más tarde";
                }
            } else {
                response.code = 0;
                ...
                if (estadoId === 1) {
                    ...
                    if (minutosParaVencer > 0) {
                        ...
                        if (resultadoUpdateUser > 0) {
                            const resultadoUpdateResetPass = await ResetPassWord.update({ ip_update: ipCliente, fecha_update: new Date(), estadoId: 2 }, {
                                where: {
                                    codigo: uuidReset
                                }
                            });
