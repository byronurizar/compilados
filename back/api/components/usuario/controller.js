...
const listAccesos = async (usuarioId) => {
    ...
    union all
    select  0 as menuId,b.accesoId from grupo_gasto_usuario a
    inner join usuario_acceso_grupo_gasto b
    on a.usuarioId=b.usuarioId where a.usuarioId=${usuarioId} and a.estadoId=1 and b.estadoId=1;`, {
        type: QueryTypes.SELECT
    });
}

const listmenu = async (usuarioId) => {
    const menuUsuario = await bd.query(`
    select distinct * from (
        select distinct a.menuId as id,a.posicion,a.descripcion as title,a.href as url,a.icono as icon,a.menu_padreId,a.classes,a.type,a.visible from cat_menu a
        inner join menu_acceso b
        ...
        union all
        select menu.menuId as id,menu.posicion,menu.descripcion as title,menu.href as url,menu.icono as icon,menu.menu_padreId,menu.classes,menu.type,1 visible from cat_menu menu
        inner join cat_acceso_grupo_gasto accesoGasto
        on menu.menuId=accesoGasto.menuId
        inner join usuario_acceso_grupo_gasto usuarioAcceso
        on accesoGasto.accesoId=usuarioAcceso.accesoId
        where menu.estadoId=1 and accesoGasto.estadoId=1 and usuarioAcceso.estadoId=1 and usuarioAcceso.usuarioId=${usuarioId}
        )as tabla  where visible=1 order by posicion asc;  `, {
        type: QueryTypes.SELECT
    });
