...
const { GrupoContable,Estado,Categoria } = require('../../../store/db');
const { registrarBitacora } = require('../../../utils/bitacora_cambios');
const { validarpermiso } = require('../../../auth');
const MenuId = 40;
const Modelo = GrupoContable;
const tabla = 'cat_grupo_contable';
let response = {};
...
const list = async (req) => {
    ...
    if (!req.query.id && !req.query.estadoId && !req.query.cargo_abono && !req.query.empresaId) {
        response.code = 1;
        response.data = await consultar(null, include);
        return response;
    }

    const { id, estadoId,cargo_abono,empresaId} = req.query;
    let query = {};
    if (estadoId) {
...
const update = async (req) => {
    ...
    const { id: grupo_contableId } = req.params;

    const dataAnterior = await Modelo.findOne({
        where: { grupo_contableId }
    });
    ...
    if (dataAnterior) {
        ...
        if (resultado > 0) {
            ...
            await registrarBitacora(req,tabla, grupo_contableId, dataAnterior.dataValues, req.body);
...
const eliminar = async (req) => {
    ...
    let grupo_contableId = req.params.id;
    const dataAnterior = await Modelo.findOne({
        where: { grupo_contableId }
    });
    ...
    if (dataAnterior) {
        ...
        if (resultado > 0) {
            ...
            await registrarBitacora(req,tabla, grupo_contableId, dataAnterior.dataValues, dataEliminar);
