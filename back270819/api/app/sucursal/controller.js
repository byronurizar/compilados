...
const { Sucursal, Estado, Municipio, Departamento } = require('../../../store/db');
...
const validarInfo = async (auxWhere = {}, texto,id=undefined) => {
    ...
    if(id){
        auxWhere.sucursalId= { [Op.ne]: id};
    }
    const existe = await Modelo.findOne({ where: auxWhere, attributes: ['sucursalId'] });
    if (existe) {
        response.code = -1;
        response.data = `El ${texto} de la sucursal ya existe por favor verifique`;
        return response;
    } else {
        return {
            code: 1,
...
const update = async (req) => {
    ...
    if (req.body.nombre) {
        ...
        query.empresaId = empresaId;
        query.nombre = req.body.nombre;
        let validacion = await validarInfo(query, 'nombre',sucursalId);
        if (validacion.code !== 1) {
            return validacion;
        }
    }
    ...
    if (dataAnterior) {
        ...
        if (resultado > 0) {
            ...
            await registrarBitacora(req, tabla, sucursalId, dataAnterior.dataValues, req.body);
...
const eliminar = async (req) => {
    ...
    if (dataAnterior) {
        ...
        if (resultado > 0) {
            ...
            await registrarBitacora(req, tabla, sucursalId, dataAnterior.dataValues, dataEliminar);
