...
const { TipoProducto,Estado} = require('../../../store/db');
const { registrarBitacora } = require('../../../utils/bitacora_cambios');
const { validarpermiso } = require('../../../auth');
const MenuId = 35;
const Modelo = TipoProducto;
const tabla = 'cat_tipo_producto';
let response = {};
...
const list = async (req) => {
    ...
    if (!id) {
        ...
        if (Number(id) > 0) {
            query.tipo_productoId = Number(id);
            response.code = 1;
            response.data = await consultar(query, include);
            return response;
        } else {
...
const update = async (req) => {
    ...
    const { id: tipo_productoId } = req.params;

    const dataAnterior = await Modelo.findOne({
        where: { tipo_productoId }
    });
    ...
    if (dataAnterior) {
        ...
        if (resultado > 0) {
            ...
            await registrarBitacora(req,tabla, tipo_productoId, dataAnterior.dataValues, req.body);
...
const eliminar = async (req) => {
    ...
    let tipo_productoId = req.params.id;
    const dataAnterior = await Modelo.findOne({
        where: { tipo_productoId }
    });
    ...
    if (dataAnterior) {
        ...
        if (resultado > 0) {
            ...
            await registrarBitacora(req,tabla, tipo_productoId, dataAnterior.dataValues, dataEliminar);
