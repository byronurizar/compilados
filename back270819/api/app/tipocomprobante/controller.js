...
const { TipoComprobante, Estado } = require('../../../store/db');
const { registrarBitacora } = require('../../../utils/bitacora_cambios');
const { validarpermiso } = require('../../../auth');
const MenuId = 0;
const Modelo = TipoComprobante;
const tabla = 'cat_tipo_comprobante';
let response = {};
...
const list = async (req) => {
    ...
    if (!id) {
        ...
        if (Number(id) > 0) {
            query.tipo_comprobanteId = Number(id);
            response.code = 1;
            response.data = await consultar(query, include);
            return response;
        } else {
...
const update = async (req) => {
    ...
    const { id: tipo_comprobanteId } = req.params;

    const dataAnterior = await Modelo.findOne({
        where: { tipo_comprobanteId }
    });
    ...
    if (dataAnterior) {
        ...
        if (resultado > 0) {
            ...
            await registrarBitacora(req,tabla, tipo_comprobanteId, dataAnterior.dataValues, req.body);
...
const eliminar = async (req) => {
    ...
    let tipo_comprobanteId = req.params.id;
    const dataAnterior = await Modelo.findOne({
        where: { tipo_comprobanteId }
    });
    ...
    if (dataAnterior) {
        ...
        if (resultado > 0) {
            ...
            await registrarBitacora(req,tabla, tipo_comprobanteId, dataAnterior.dataValues, dataEliminar);
