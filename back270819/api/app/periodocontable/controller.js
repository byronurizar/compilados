...
const { PeriodoContable,Estado } = require('../../../store/db');
const { registrarBitacora } = require('../../../utils/bitacora_cambios');
const { validarpermiso } = require('../../../auth');
const MenuId = 45;
const Modelo = PeriodoContable;
const tabla = 'cat_periodo_contable';
let response = {};
...
const list = async (req) => {
    ...
    if (!id) {
        ...
        if (Number(id) > 0) {
            query.periodo_contableId = Number(id);
            response.code = 1;
            response.data = await consultar(query, include);
            return response;
        } else {
...
const update = async (req) => {
    ...
    const { id: periodo_contableId } = req.params;

    const dataAnterior = await Modelo.findOne({
        where: { periodo_contableId }
    });
    ...
    if (dataAnterior) {
        ...
        if (resultado > 0) {
            ...
            await registrarBitacora(req,tabla, periodo_contableId, dataAnterior.dataValues, req.body);
...
const eliminar = async (req) => {
    ...
    let periodo_contableId = req.params.id;
    const dataAnterior = await Modelo.findOne({
        where: { periodo_contableId }
    });
    ...
    if (dataAnterior) {
        ...
        if (resultado > 0) {
            ...
            await registrarBitacora(req,tabla, periodo_contableId, dataAnterior.dataValues, dataEliminar);
