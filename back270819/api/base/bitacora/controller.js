...
cambios = async (req) => {
    ...
    query=`bitacora_cambios.empresaId=${empresaId}`
    if (tabla) {
        query = ` and bitacora_cambios.tabla='${tabla}'`;
    }
...
peticiones = async (req) => {
    ...
    query=`bitacora_peticion.empresaId=${empresaId}`;
    if (baseUrl) {
        query = ` and bitacora_peticion.baseUrl='${baseUrl}'`;
...
peticionesRequest = async (req) => {
    ...
    if (bitacora_peticionId>0) {
        ...
        response.data = await BitacoraPeticion.findOne({
            where:{bitacora_peticionId,empresaId},
            attributes: ["request"],
        });
    } else {
        response.code = -1;
