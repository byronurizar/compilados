...
const consultar = async (query, include = 1) => {
    if (include == 1) {
        ...
        if (query) {
            return await Modelo.findAll({ where: sequelize.literal(query) });
        } else {
            return await Modelo.findAll();
        }
    }
...
list = async (req) => {
    ...
    let query ="";
    query=` usuario.personaId in(select personaId from persona where persona.empresaId=${empresaId})`;
    ...
    if (estadoId) {
        ...
        let auxInSelect=' and usuario.estadoId in(';
        estados.map((item) => {
        ...
        auxInSelect+=')';
        auxInSelect=auxInSelect.replace(",)",")")
        query+=auxInSelect;
    ...
    if (personaId) {
        ...
        query=` and usuario.personaId=${personaId}`
    }
    ...
    if (!id) {
        ...
        if (Number(id) > 0) {
            query=`and usuario.usuarioId=${id}`
            response.code = 1;
            response.data = await consultar(query, include);
            return response;
        } else {
...
const eliminar = async (req) => {
    ...
    if (dataAnterior) {
        ...
        if (resultado > 0) {
            ...
            await registrarBitacora(req,tabla, usuarioId, dataAnterior.dataValues, dataEliminar);
...
const update = async (req) => {
    ...
    if (dataAnterior) {
        ...
        if (resultado > 0) {
            ...
            await registrarBitacora(req,tabla, usuarioId, dataAnterior.dataValues, req.body);
            //Actualizar fecha de ultima modificacion
            let fecha_ult_mod = moment(new Date()).format('YYYY/MM/DD HH:mm');
            const data = {
                fecha_ult_mod,
                    ...
                    .then(async (sonIguales) => {
                        if (sonIguales === true) {
                            ...
                            if (resultado > 0) {
                                ...
                                await registrarBitacora(req,tabla, usuarioId, dataAnterior.dataValues, dataUpdate);
                                //Actualizar fecha de ultima modificacion
                                let fecha_ult_mod = moment(new Date()).format('YYYY/MM/DD HH:mm');
                                const data = {
                                    fecha_ult_mod,
