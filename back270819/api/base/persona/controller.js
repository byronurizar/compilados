...
const eliminar = async (req) => {
    ...
    if (dataAnterior) {
        ...
        if (resultado > 0) {
            ...
            await registrarBitacora(req,tabla, personaId, dataAnterior.dataValues, dataEliminar);
...
const update = async (req) => {
    ...
    if (dataAnterior) {
        ...
        if (resultado > 0) {
            ...
            await registrarBitacora(req,tabla, personaId, dataAnterior.dataValues, req.body);
            //Actualizar fecha de ultima modificacion
            let fecha_ult_mod = moment(new Date()).format('YYYY/MM/DD HH:mm');
            const data = {
                fecha_ult_mod,
...
const importar=async(req)=>{
    let { file } = req;
    if(!file){
        response.code=-1;
        response.data="No se adjunto ningún archivo"
        return response;
    }
    var exceltojson;
    if(file.originalname.split('.')[req.file.originalname.split('.').length-1] === 'xlsx'){
